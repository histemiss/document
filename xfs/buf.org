...
** xfs_buf_item_size_segment(xfs_buf_log_item, xfs_buf_log_format)
   - 遍历xfs_buf_log_format->blf_data_map, 里面连续为1的位使用一个xfs_log_iovec. 查找需要的xfs_log_iovec数量.
   ...
   - 还需要统计占用的log队列空间, 第一个是xfs_buf_log_format_size, 每一位使用XFS_BLF_CHUNK/128

** xfs_buf_item_size(xfs_log_item, nvecs, nbytes)
   - 为xfs_log_item创建xfs_log_vec时使用, 计算xfs_log_iovec的个数,以及log数据的大小

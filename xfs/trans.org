...
** xlog_state_do_callback(xlog, aborted, xlog_in_core)
   ...
   - 从当前xlog_in_core下一个开始遍历, 相当于队尾到对头, 这个顺序时按照提交的先后顺序进行
   - 可能涉及到cil/ail的顺序.
   ...
   - 结束遍历, 同时把当前的状态从XLOG_STATE_DONE_SYNC改为XLOG_STATE_DO_CALLBACK, 其他xlog_in_core可以处理它
   - 否则获取一个xlog_in_core, 它的状态是XLOG_STATE_DONE_SYNC或者XLOG_STATE_DO_CALLBACK
   - 获取xlog的最小lsn
   > xlog_get_lowest_lsn(xlog)
   - 如果xlog_in_core->xlog_op_header->h_lsn不是最小的,也不处理, 可能前面有ioerror的xlog_in_core
   - 到这里就只处理lsn最小的,而且状态是XLOG_STATE_DONE_SYNC|XLOG_STATE_DO_CALLBACK状态的, 改为XLOG_STATE_CALLBACK
   - 这样保证callback的有序
   - 如果xlog_in_core->ic_callback != 0
   - 更新xlog->l_last_sync_lsn = xlog_in_core->xlog_in_core2->xlog_rec_header->h_lsn

...
** xfs_bmbt_rec_host
   ...
   - 对于XFS_DINODE_FMT_EXTENTS,读取xfs_dinode时把xfs_bmbt_rec读取出来, 一种直接设置XFS_IFEXTENTS标志
   - 对于XFS_DINODE_FMT_BTREE,需要专门读取 (xfs_iread_extents),使用xfs_ifork->fi_flags的XFS_IFBROOT表示,读取只会才设置XFS_IFEXTENTS
   ...
   - 2维数组使用xfs_ext_irec, 里面可能不是满的,使用中需要压缩整理

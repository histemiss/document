...
** xfs_bmap_extents_to_btree(xfs_trans, xfs_inode, xfs_fsblock_t firstblock, xfs_bmap_free, xfs_btree_cur, wasdel, logflags, whichfork)
   ...
   - 设置xfs_alloc_args->type = XFS_ALLOCTYPE_START_BNO
   - 设置xfs_alloc_args->fsbno = XFS_INO_TO_FSB(xfs_mount, xfs_inode->i_ino)
   - 如果不是NULLFSBLOCK, 而且xfs_bmap_free->xbf_low !=0, 分配连续的空间? 
   - 设置xfs_alloc_args->type = XFS_ALLOCTYPE_START_BNO
   - 设置xfs_alloc_args->fsbno = firstblock
   - 否则就近分配?
   - 设置xfs_alloc_args->type = XFS_ALLOCTYPE_NEAR_BNO
   - 设置xfs_alloc_args->fsbno = firstblock
   - 设置xfs_alloc_args->minlen = maxlen = prod = 1
...
** xfs_bmap_btalloc(xfs_bmalloca)
   ...
   - 设置xfs_alloc_args->type 为XFS_ALLOCTYPE_START_BNO??
   ...
   - 设置xfs_alloc_args->type = XFS_ALLOCTYPE_THIS_BNO, alignment = 1, tryagain = 1, 失败后会尝试别的??
